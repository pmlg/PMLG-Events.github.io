...
The bundle includes `index.html`, `404.html`, `.nojekyll`, compiled JavaScript and CSS, the PMLG logo, `upcoming-events.json`, `upcoming-event-template.json`, and the runtime-loaded `events-archive.json`. It intentionally excludes raw MHTML source pages, project source files, development tooling, and server code.
...
Edit the root-level `upcoming-events.json` file in the GitHub Pages repository. Keep it as a JSON array and copy the shape from `upcoming-event-template.json`. Add one event object per listing, commit, and push. The site fetches this file at runtime, so adding or changing an upcoming event does not require rebuilding the JavaScript bundle. Put each event’s specific Eventbrite ticket page in its `url` field; if omitted, the sign-up button opens the general PMLG Eventbrite organiser page. LinkedIn remains available for general community contact; no public email address is stored in the site.
