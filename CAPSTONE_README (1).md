...
The platform is engineered as a client-side rendered single-page application (SPA) backed by lightweight JSON data stores and automated GitHub Actions. Eventbrite is the ticketing provider for new events, while LinkedIn is retained for general community contact. By eliminating backend server infrastructure, server-side database maintenance, and recurring hosting fees, the site achieves zero operational cost while maintaining strict security and privacy standards.
...
- **Privacy-First Communication**: All public email addresses and `mailto:` links have been removed. Event ticketing routes through Eventbrite, using each event’s specific URL when provided and the PMLG organiser page as fallback; LinkedIn remains the general community/contact channel.
...
export interface PmlgEvent {
  ...
  url?: string;               // Eventbrite ticket page for upcoming events
